...
function Run(arrow) {
  ...
  let imgWidth = img.width;
  focus = (focus + 1) % 2;
  direction = checkPageBounds(arrow, direction, imgWidth, pos, pageWidth); 
  img.src = pacArray[direction][focus];
  if (direction) {
    if (pos <= 0) pos = 0;
    else pos -= 20;
    ...
    if (pos >= pageWidth-imgWidth) pos = pageWidth-imgWidth;
    else pos += 20;
    img.style.left = pos + 'px';
  }
}
...
document.addEventListener('keydown', (event) => {
  ...
  if (keyName === 'ArrowRight' || keyName === 'ArrowLeft') Run(keyName);
});
// This function determines the direction of PacMan based on screen edge detection. 
function checkPageBounds(arrow, direction, imgWidth, pos, pageWidth) {
  //
  // TODO: Complete this to reverse direction upon hitting screen edge
  //
  
  if (arrow === 'ArrowRight') direction = 0;
  else if (arrow === 'ArrowLeft') direction = 1;
